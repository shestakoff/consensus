function Results = RunConsensus(DataArr, ConsFuncList, Options)
...
for idata = 1:numData
    ...
    for iExp = 1:numExper;
        ...
        [A, P, I, C, S, AS] = BuildEnsembleFormats(DataArr{idata}.Ensemble{iExp});
        for ifunc = 1:numFunc
            tic
            fprintf('--- Starting %s...\n', ConsFuncList{ifunc,1});
            
            % Special for LinkCLue and bayes
            ...
            if Options.IsKnownK && ...
                    (strcmp(ConsFuncList{ifunc,1}, 'ARA\_All\_mean\_noClDiag\_Diag') ||...
                    strcmp(ConsFuncList{ifunc,1}, 'ARA\_All\_CTS\_mean\_noClDiag\_Diag'));
                ConsFuncList{ifunc,3}.AlgorithmParams.ClusterNumLimit = length(unique(DataArr{idata}.truth));
            end
