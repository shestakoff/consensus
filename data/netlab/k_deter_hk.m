function [sk_hk,sk_len_hk,stand_hk,real_hk]=k_deter_hk(data,y,k_start,k_end,rows,v,times_app)
[R,C]=size(y);
ks=k_end-k_start+1;
times=1;
for i=k_start:k_end
    for dif_init=1:times_app
        ...
        [clus_liste,cent,re_cent]=straight_kmeans(data,y,sk,i,cent_i,cent_mo);
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for iter=1:times_app
        for ii=1:k_nume(iter)
            wsc{iter}(ii,:)=sum(sum((y(kliste{iter}(ii,1:len_clus_liste{iter}(ii))',:)-repmat(stand_cente{iter}(ii,:),len_clus_liste{iter}(ii),1)).^2,2),1);
        end
    end
    for iter=1:times_app
    ...
    stand_cent{times}=stand_cente{inx};
...
for iter=1:ks
    for i=1:k_num(iter)
        wsc1{iter}(i,:)=sum(sum((y(klist{iter}(i,1:len_clus_list{iter}(i))',:)-repmat(stand_cent{iter}(i,:),len_clus_list{iter}(i),1)).^2,2),1);
    end
end
for iter=1:ks
