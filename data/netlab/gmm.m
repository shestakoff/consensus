function mix = gmm(dim, ncentres, covar_type, ppca_dim, rows, card, sigma_square, sigma_shift , discen)
...
vartypes = {'spherical', 'diag','diag2', 'full', 'ppca'};
...
if(ischar(card))
    switch card
        case 'eq'
            % Initialise priors to be equal
            mix.priors = ones(1,mix.ncentres) ./ mix.ncentres;
        case 'rand'
            % % Initialise priors to be randomly different and sum to one
            b(1)=0;b(mix.ncentres+1)=1;
            for i=2:mix.ncentres
                b(i)=rand;
                while b(i)<0.1
                    b(i)=rand;
                end
            ...
            for i=1:mix.ncentres
                mix.priors(i)=b(i+1)-b(i);
            end
    end
else    
    mix.priors = card/sum(card);
end
% Initialise centres
mix.centres = randn(mix.ncentres, mix.nin).*discen;
...
switch mix.covar_type
    ...
    case 'diag'
        ...
        mix.covars =  sigma_square*ones(mix.ncentres, mix.nin);
        mix.nwts = mix.ncentres + mix.ncentres*mix.nin + ...
            mix.ncentres*mix.nin;
    case 'diag2'
        % Store diagonals of covariance matrices as rows in a matrix
        mix.covars =  (rand(mix.ncentres, mix.nin)*sigma_square+sigma_shift);
        mix.nwts = mix.ncentres + mix.ncentres*mix.nin + ...
            mix.ncentres*mix.nin;
    case 'full'
    ...
    case 'ppca'
        ...
        % lambdas
        mix.covars = sigma_square*ones(1, mix.ncentres);
        ...
        %  vr=ones(mix.centres,mix.ppca_dim);
        %  vt=[0:mix.ppca_dim];
        %  vt=mix.ppca_dim-vt;
        %  vt(mix.ppca_dim)=[];
        %  for ii=1:mix.ncentres
        %   mix.lambda=[1 2 3 4 5];
        %  end
