function [clus_list,cent,real_cent]=straight_kmeans(data,y,sk,k,cent_sq,cent_mo)
[R,C]=size(data);
if cent_mo==0
    [R_k,C_k]=size(sk);
    clus_list=sk;
    ...
    for i=1:j-1
        n=len_clus_list(i);
        cent(i,:)=sum(y(clus_list(i,1:n)',:),1)/n;
    end
elseif cent_sq==0
    [R_k,C_k]=size(sk);
    clus_list=sk;
    ...
    for i=1:k
        n=len_clus_list(i);
        cent(i,:)=median(y(clus_list(i,1:n)',:),1);
    end
end