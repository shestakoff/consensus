function [sk,new_len_clus_list,stand_cent,real_cent]=anomalous_pattern_kmeans(data,y,least_square,least_moduli,re_threshold)
[R,C]=size(y);
tmp1=y;
tmp2=zeros(R,1);
t=1;
...
while z~=0
    [clus_list,cent]=anomalous_pattern(tmp1,z,least_square,least_moduli);
...
for i=1:k
    ...
    if least_square==1
        cent_ksq(i,:)=sum(y(tmp3(i,1:n)',:))/n;   %the remaining centroids  least_square
        cent_kmo=0;
    end
    if least_moduli==1
        cent_kmo(i,:)=median(y(tmp3(i,1:n)',:),1);   %the remaining centroids   least_moduli
        cent_ksq=0;
    end
end
[sk,stand_cent,real_cent]=straight_kmeans(data,y,tmp3,k,cent_ksq,cent_kmo);
[R_sk,C_sk]=size(sk);
for i=1:R_sk
    m=0;
    for j=1:C_sk
